<template>
   <div id="account" class="flexv wrap">
      <tab active-color="#fe4400" custom-bar-width="4rem" :line-width=3 v-model="indexs" class="nav">
         <tab-item class="flex center" :selected="index==indexs" v-for="(item, index) in headText" @click="indexs=index" :key="index"><span class="flex center">{{item}}</span></tab-item>
      </tab>
      <swiper v-model="indexs" class="flexitem" :show-dots="false">
         <swiper-item class="flexv wechat">
            ...
            <div class="flexitemv centerv qrcodebox" v-if="accredit && keep">
            ...
            <form class="flexv centerv tab-swiper vux-center" @submit.prevent="sendForm">
               <div class="flexv bg_white content">
                  <label class="flex list">
                     ...
                     <input type="text" name="ali_name" class="flexitem" v-model="ali_name" placeholder="请填写支付宝姓名" data-rule="*" data-errmsg="请填写姓名" data-sync="true">
                  </label>
                  <label class="flex list">
                     <span class="flex center">支付宝账户</span>
                     <input type="text" name="ali_account" class="flexitem" v-model="ali_account" placeholder="请填写支付宝账户" data-rule="*" data-errmsg="请填写账户" data-sync="true">
                  </label>
               </div>
   ...
   export default {
      ...
      created(){
         ...
         this.$store.dispatch('user_listing').then(user => {
            this.keep = user.is_subscribe != 0;
            this.ali_name = user.ali_name;
