<template>
   <div id="cliper" class="clipper-container" ref="clipper">
      <!-- 头部操作栏 -->
      <div class="action-bar">
         <a href="javascript:;" class="btn-cancel" @click="_cancel">取消</a>
         <div class="title">裁剪头像</div>
         <a href="javascript:;" class="btn-ok" @click="_clipper">确认</a>
      </div>
      <div class="cliperbox">
...
<script>
   export default{
      ...
      methods: {
         ...
         _initEvent() {
            let _this2 = this,
               $gesture = this.$refs.gesture,
               cClientRect = this.$refs.canvas.getBoundingClientRect(),
               scx = 0,
               //对于单手操作是移动的起点坐标，对于缩放是图片距离两手指的中点最近的图标。
               scy = 0,
               fingers = {}; //记录当前有多少只手指在触控屏幕

            //one finger
            let iX = this.imgX, iY = this.imgY;

            //two finger
            let figureDistance = 0, pinchScale = this.imgScale;

            $gesture.addEventListener('touchstart', function (e) {
               if (!_this2.imgLoaded) return;
               ...
               if (e.touches.length === 1) {
                  ...
                  figureDistance = _this2._pointDistance(f1x, f1y, f2x, f2y);
                  ...
                  if (scx > _this2.imgX + _this2.imgCurrentWidth) {
                     scx = _this2.imgX + _this2.imgCurrentHeight;
                  }
                  if (scy < _this2.imgY) {
                     scy = _this2.imgY;
                  }
                  if (scy > _this2.imgY + _this2.imgCurrentHeight) {
                     scy = _this2.imgY + _this2.imgCurrentHeight;
                  }
               }
            }, false);
            $gesture.addEventListener('touchmove', function (e) {
               ...
               if (e.touches.length === 1) {
                  ...
                  _this2._drawImage(iX + f1x - scx, iY + f1y - scy, _this2.imgCurrentWidth, _this2.imgCurrentHeight);
               } else if (e.touches.length === 2) {
                  let finger1 = e.touches[0],
                     finger2 = e.touches[1],
                     _f1x = finger1.pageX - cClientRect.left,
                     _f1y = finger1.pageY - cClientRect.top,
                     f2x = finger2.pageX - cClientRect.left,
                     f2y = finger2.pageY - cClientRect.top,
                     newFigureDistance = _this2._pointDistance(_f1x, _f1y, f2x, f2y),
                     scale = _this2.imgScale + parseFloat(((newFigureDistance - figureDistance) / _this2.imgScaleStep).toFixed(1));
                  ...
                  if (scale !== pinchScale) {
                     //目前缩放的最小比例是1，最大是5
                     if (scale < _this2.imgMinScale) {
                        scale = _this2.imgMinScale;
                     } else if (scale > _this2.imgMaxScale) {
                        scale = _this2.imgMaxScale;
                     }

                     pinchScale = scale;
                     _this2._scale(scx, scy, scale);
                  }
               }
            }, false);
            $gesture.addEventListener('touchend', function (e) {
               ...
               let x = _this2.imgX,
                  y = _this2.imgY,
                  pClientRect = _this2.$refs.pCanvas.getBoundingClientRect();

               //左右边界
               if (x > pClientRect.left) {
                  x = pClientRect.left;
               } else if (x + _this2.imgCurrentWidth < pClientRect.left + pClientRect.width) {
                  x = pClientRect.left + pClientRect.width - _this2.imgCurrentWidth;
               }
               //上下边界 (52为头部的高度)
               if (y > pClientRect.top - 52) {
                  y = pClientRect.top - 52;
               } else if (y + _this2.imgCurrentHeight < pClientRect.top - 52 + pClientRect.height) {
                  y = pClientRect.top - 52 + pClientRect.height - _this2.imgCurrentHeight;
               }

               if (_this2.imgX !== x || _this2.imgY !== y) {
                  _this2._drawImage(x, y, _this2.imgCurrentWidth, _this2.imgCurrentHeight);
               }
            });
         },
         _loadImg() {
            ...
            let $img = new Image(),
               onLoad = function onLoad(e) {
                  ...
                  _this3.$img = $img;
                  _this3.imgLoaded = true;
                  _this3.imgLoading = false;

                  _this3._initImg($img.width, $img.height);
                  _this3.$emit('loadSuccess', e);
                  _this3.$emit('loadComplete', e);
                  _this3._loadImg();
               },
...
<style scoped>
   #cliper{position:fixed;top:0;bottom:0;left:0;right:0;z-index:100;line-height:0;background:#fff}
   #cliper .action-bar{height:52px;line-height:52px;background:#fdfdfd;border-bottom:1px solid #eee;padding:0 60px}
   #cliper .action-bar a{position:absolute;top:0;width:60px;text-align:center;bottom:0}
   #cliper .action-bar a.btn-cancel{left:0;color:red}
   #cliper .action-bar a.btn-ok{right:0;color:#27e}
   #cliper .action-bar .title{text-align:center;font-size:1.05em}
   #cliper .cliperbox{position:absolute;top:52px;left:0;right:0;bottom:0}
   #cliper .mask{position:absolute;top:0;bottom:0;left:0;right:0;z-index:100;background-color:rgba(0,0,0,.5)}
   #cliper .clipper-part{position:absolute;top:0;bottom:0;left:0;right:0;z-index:101}
   #cliper .clipper-part .pCanvas-container{position:absolute;top:50%;left:50%;-webkit-transform:translate(-50%,-50%);transform:translate(-50%,-50%);border:2px dashed #ddd}
   #cliper .gesture-mask{position:absolute;top:0;bottom:0;left:0;right:0;z-index:102}
</style>
